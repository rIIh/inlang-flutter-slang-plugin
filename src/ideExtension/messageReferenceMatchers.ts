...
const createParser = () => {
	return Parsimmon.createLanguage({
		...
		findReference: function (r) {
			return Parsimmon.seq(Parsimmon.regex(/import '.*strings.*.dart';/s), r.findMessage!.many().map((matches) => matches.flatMap((match) => match)))
		},

		findMessage: () => {
			return Parsimmon.seqMap(
				Parsimmon.regex(/.*?(?<![a-zA-Z0-9/])context\.t\./s), // no preceding letters or numbers
				...
				(_, start, messageId, end, args) => {
					...
					return [{
						messageId: `${parts[0]}:${parts.slice(1, parts.length).join('.')}`,
						position: {
							start: {
								line: start.line,
